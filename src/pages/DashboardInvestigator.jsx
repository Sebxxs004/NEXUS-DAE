...
function DashboardInvestigator({ token }) {
  ...
  const { usuario, logout, completarPrimeraVez, guardarTiempo, obtenerPerfil, guardarEventos } = useAuthStore();
  ...
  const [pendingDecisionGroups, setPendingDecisionGroups] = useState([]);
  ...
  const [shownEventIds, setShownEventIds] = useState(() => {
    try {
      return usuario?.shown_event_ids ? JSON.parse(usuario.shown_event_ids) : [];
    } catch (e) {
      return [];
    }
  });
  ...
  useEffect(() => {
    ...
    if (secondsSinceLast >= requiredDelay) {
      ...
      if (available.length > 0) {
        ...
        setEventReplyText('');
        const nextEvents = [...shownEventIds, chosen.id];
        setShownEventIds(nextEvents);
        guardarEventos(nextEvents).catch(console.error);
        setLastEventTriggeredTime(elapsedSeconds); // Anchor next event relative to this moment
      }
    }
  }, [elapsedSeconds, despachoEvents, shownEventIds, investigationFinished, tutorialStep, lastEventTriggeredTime, activeEvent, guardarEventos]);
  ...
  const startDecisionFlow = () => {
    ...
    // Filter only groups that don't have decisions yet
    const undecided = createdGroupsList.filter((g) => !g.decisions || g.decisions.length === 0);
    if (undecided.length === 0) {
      setPendingDecisionGroups([]);
      setDecisionFlowCompleted(true);
      setIsDecisionFlowModalOpen(true);
      return;
    }
    setPendingDecisionGroups(undecided);
  ...
  const handleNextDecisionGroup = () => {
    ...
    // Save decisions for the current group
    const currentGroup = pendingDecisionGroups[activeDecisionGroupIndex];
    if (currentGroup) {
      const decisionsForGroup = selectedDecisionOptions.map((optId) => ({
        optionId: optId,
        label: DECISION_OPTIONS.find((o) => o.id === optId)?.label || '',
        justification: decisionJustifications[optId] || ''
      }));
      ...
      const updatedGroups = createdGroupsList.map((g) => {
        if (g.id === currentGroup.id) {
          return { ...g, decisions: decisionsForGroup };
        }
        return g;
      });
    ...
    if (activeDecisionGroupIndex < pendingDecisionGroups.length - 1) {
        ...
        >
          ...
          <div className="flex flex-col md:flex-row items-stretch md:items-end justify-between w-full gap-4 pb-2">
            ...
            <button
              ...
              className="flex flex-col items-center justify-center p-5 w-full md:w-72 h-36 rounded-xl border border-emerald-500/20 bg-slate-950/80 hover:border-emerald-400 hover:bg-slate-900/90 transition-all duration-300 text-center space-y-2 group shadow-xl hover:shadow-[0_0_20px_rgba(16,185,129,0.15)]"
            >
